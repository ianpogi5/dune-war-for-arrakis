...
def area_meta(v):
    if v in NAMED:
        name,terr,deep,settle,sietch = NAMED[v]
        return ts_str(name), ts_str(terr), ('true' if deep else 'false'), (str(settle) if settle else 'null'), ('true' if sietch else 'false')
    terr = KNOWN_UNNAMED.get(v)
    return 'null', ts_str(terr), ('false' if terr else 'null'), 'null', 'false'
...
o.append('//')
o.append('// Source of truth: BOARD_VERIFICATION.md (§1 types, §2 sectors, §3a adjacency, §4 impassable, §5 air zones).')
o.append('// NOTE: terrain/deep are null for most UNNAMED areas — not yet individually typed (a known gap;')
o.append('//       the adjacency graph itself is complete and verified). Fill via a later terrain pass.')
o.append('')
